...
func (s *StepStopInstance) Run(state multistep.StateBag) multistep.StepAction {
	...
	err := instanceClient.ActionStop(instance.ID.String(), false, 60)
	if err != nil {
	...
	stateConf := &StateChangeConf{
		...
		Refresh:   InstanceRefreshPowerFunc(instanceClient, instance.ID.String()),
		StepState: state,
	}
	_, err = stateConf.WaitForState()
	if err != nil {
		err := fmt.Errorf("Error waiting for instance (%s) to stop: %s", instance.ID.String(), err)
		state.Put("error", err)
		ui.Error(err.Error())
		return multistep.ActionHalt
	}
...
func InstanceRefreshPowerFunc(instanceClient client.InstanceClientInterface, instanceID string) func() (result interface{}, state string, err error) {
